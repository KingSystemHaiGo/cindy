...
const MEMORY_GUIDANCE = [
  ...
  '也记住重要的「时刻」:用户的一次重要想法或方向转变、你们一起经历的重大节点 (发布、里程碑、重大纠偏)、用户明确让你记住的话。这类内容用 memory_write 记成 type moment,写清楚发生了什么、当时怎么说的;可以带 occurredAt (事件发生的日期) 和 significance (重大时刻用 high)。',
  '时刻是「当时的事」,偏好是「一直以来」;两类分开记,不互相混写。流水账、临时状态、过几天就过期的进度,照旧不进记忆。',
].join('\n');
