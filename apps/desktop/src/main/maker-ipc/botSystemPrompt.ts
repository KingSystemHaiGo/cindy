...
function buildMemoryGuidance(memoryToolName = DEFAULT_MEMORY_WRITE_TOOL): string {
  return [
    ...
    `重要的「时刻」也走同一道门槛,不得因为用户随口说了一个想法或当天过了一个节点就记:用户明确要求记住的话、已经反复确认过的方向转变、或你们一起完成且以后还用得上的重大节点(发布、里程碑、重大纠偏)。${momentWriteGuidance(memoryToolName)},写清楚发生了什么、当时怎么说的;可以带 occurredAt (事件发生的日期) 和 significance (重大时刻用 high)。`,
